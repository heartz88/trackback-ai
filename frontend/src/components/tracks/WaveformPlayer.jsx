...
if (!spectrumRef.current || !audioRef.current) return;
const w = spectrumRef.current.getBoundingClientRect().width;
if (w < 10) return;
audioMotionInitDone.current = true;
initAudioMotion(audioRef.current);
};
...
const poll = setInterval(() => {
    attempts++;
    tryInitAudioMotion.current?.();
    if (audioMotionInitDone.current || attempts > 50) clearInterval(poll);
}, 100);
