...
const { user } = useAuth();
const {
track, owner, collaboration, collaborators, submissionsCount,
isLoading, error, requestingCollab, collabMessage, setCollabMessage,
showMessageInput, setShowMessageInput, isPlaying, handlePlay, handlePause,
isOwner, isCompleted, hasApprovedCollab, canRequestCollab,
handleRequestCollaboration, handleCompleteTrack,
} = useTrackDetail(trackSlug);
...
<div className="track-detail-page animate-fade-in">
    ...
    <div className="tdp-info animate-slide-up stagger-2">
        ...
        {isCompleted && (
            ...
            padding:'3px 10px', borderRadius:9999, background:'rgba(20,184,166,0.12)',
            color:'var(--accent-primary)', border:'1px solid rgba(20,184,166,0.35)' }}>
            ✅ Completed
            </span>
        )}
        </div>
    ...
    <div className="tdp-actions-panel animate-slide-up stagger-3">
        ...
        {!isCompleted && (<>
            ...
            {showMessageInput && (
                ...
                style={{ width:'100%', boxSizing:'border-box', padding:'10px 12px', background:'var(--surface-2)',
                    border:'1px solid var(--surface-border)', borderRadius:'var(--radius-md)',
                    color:'var(--text-primary)', fontSize:13, resize:'none', outline:'none', fontFamily:'inherit' }} />
            )}
        ...
        <div className="tdp-action-card">
            ...
            {collaborators.map(collab => (
                ...
                <Link to={`/profile/${collab.username}`} className="flex items-center gap-3 flex-1 min-w-0 group">
                    ...
                    <div className="font-semibold text-[var(--text-primary)] text-sm group-hover:text-primary-400 truncate">@{collab.username}</div>
    ...
    <div className="tdp-section-head">
        ...
        {submissionsCount > 0 && (
        <Link to={`/tracks/${trackSlug}/submissions`} className="btn-view-all">View All ({submissionsCount}) →</Link>
        )}
    </div>
    ...
    {submissionsCount === 0 && !isCompleted && (
        ...
        No submissions yet.{' '}
        {hasApprovedCollab && (
            <Link to={`/tracks/${trackSlug}/submissions`} style={{ color:'var(--accent-primary)' }}>Be the first!</Link>
        )}
        </div>
    )}
    </div>
