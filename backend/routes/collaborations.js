...
// Get user's collaboration for a specific track (for TrackDetailPage & SubmissionsPage)
router.get('/track/:trackId', authMiddleware, async (req, res) => {
try {
    const { trackId } = req.params;
    const userId = req.user.id;

    console.log(`🔍 Getting collaboration for track ${trackId}, user ${userId}`);

    const result = await db.query(
        `SELECT cr.*, t.title as track_title, t.user_id as track_owner_id
            FROM collaboration_requests cr
            JOIN tracks t ON cr.track_id = t.id
    ...
    console.log(`✅ Found collaboration: ${result.rows[0].status}`);
...
// Get active collaborations for a user (for ProfilePage)
router.get('/active/:userId', authMiddleware, async (req, res) => {
try {
    const { userId } = req.params;

    console.log(`🤝 Getting active collaborations for user ${userId}`);
    ...
    const result = await db.query(
        `SELECT 
            ...
            t.title as track_title,
            CASE 
                WHEN ac.owner_id = $1 THEN collaborator.username
                ELSE owner.username
            ...
            JOIN users collaborator ON ac.collaborator_id = collaborator.id
    ...
    console.log(`✅ Found ${result.rows.length} active collaborations`);
    ...
    res.status(500).json({ 
        error: { 
            message: 'Failed to fetch active collaborations',
            details: process.env.NODE_ENV === 'development' ? error.message : undefined
        } 
    });
}
