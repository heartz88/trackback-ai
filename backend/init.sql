...
INSERT INTO users (username, email, password_hash, bio, skills, social_links, looking_for_collab, last_active)
SELECT 
'admin_user',
'admin@trackback.ai',
...
-- Function to get user collaboration stats (updated with new columns)
...
SELECT 
    ...
    -- Looking for collaboration status
    (SELECT looking_for_collab FROM users WHERE id = p_user_id) as looking_for_collab,
    
    -- Last active timestamp
    (SELECT last_active FROM users WHERE id = p_user_id) as last_active;
END;
$$ LANGUAGE plpgsql;
...
u.looking_for_collab as owner_looking_for_collab,
...
u.skills,
u.social_links,
u.looking_for_collab,
u.last_active,
u.created_at,
(SELECT COUNT(*) FROM tracks WHERE user_id = u.id) as total_tracks,
(SELECT COUNT(*) FROM collaboration_requests cr
    JOIN tracks t ON cr.track_id = t.id
    WHERE (t.user_id = u.id OR cr.collaborator_id = u.id)
    AND cr.status = 'approved') as total_collaborations
FROM users u;
